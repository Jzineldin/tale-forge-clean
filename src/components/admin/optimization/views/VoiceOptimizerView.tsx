...
const VoiceOptimizerView: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [metrics, setMetrics] = useState<PerformanceMetric[]>([]);
  const [events, setEvents] = useState<OptimizationEvent[]>([]);
  const [config, setConfig] = useState(configManager.getOptimizerConfig(OptimizerType.VOICE));
  const [featureFlags, setFeatureFlags] = useState(featureFlagManager.getAllFlags().filter(
    flag => flag.name.startsWith('voice-')
  ));
  ...
  useEffect(() => {
    ...
    const eventListener = (event: OptimizationEvent) => {
      if (event.optimizerType === OptimizerType.VOICE) {
  ...
  const loadData = async () => {
    ...
    try {
      // Get metrics
      setMetrics(voiceOptimizerAdapter.getMetrics());
      
      // Get events
      const voiceEvents = Array.from(events)
        .filter(event => event.optimizerType === OptimizerType.VOICE)
        .sort((a, b) => b.timestamp - a.timestamp);
      setEvents(voiceEvents);
      
      // Get config
      setConfig(configManager.getOptimizerConfig(OptimizerType.VOICE));
      ...
      // Get cache stats
      if (typeof voiceOptimizerAdapter.getEnhancedCacheStats === 'function') {
        setCacheStats(voiceOptimizerAdapter.getEnhancedCacheStats());
      }
    } catch (error) {
      console.error('Error loading voice optimizer data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveConfig = (newConfig: any) => {
    configManager.updateOptimizerConfig(OptimizerType.VOICE, newConfig);
    setConfig(configManager.getOptimizerConfig(OptimizerType.VOICE));
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center">
          ...
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-bold text-white">Voice Optimizer</h2>
          </div>
        </div>
        <button 
          onClick={loadData}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-800 rounded-lg p-4 shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-slate-700 rounded-lg p-3">
                ...
                <div className="flex items-center">
                  {voiceOptimizerAdapter instanceof Object ? (
                    <CheckCircle className="h-5 w-5 text-green-500 mr-2" />
                  ) : (
                    <XCircle className="h-5 w-5 text-red-500 mr-2" />
                  )}
                  <span className="text-white">
                    {voiceOptimizerAdapter instanceof Object ? 'Initialized' : 'Not Initialized'}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-700 rounded-lg p-3">
                  ...
                  <div className="text-gray-400 text-xs mt-1">
                    {((cacheStats.size / cacheStats.maxSize) * 100).toFixed(1)}% of max ({(cacheStats.maxSize / (1024 * 1024)).toFixed(2)} MB)
                  </div>
                </div>
                
                <div className="bg-slate-700 rounded-lg p-3">
                  <div className="text-gray-400 text-sm mb-1">Cached Assets</div>
                  <div className="text-white text-xl font-semibold">
                    {cacheStats.assetCount}
                  </div>
                </div>
                
                {cacheStats.formatDistribution && Object.keys(cacheStats.formatDistribution).length > 0 && (
                  <div className="bg-slate-700 rounded-lg p-3 md:col-span-2">
                    ...
                    <div className="flex items-center space-x-2">
                      {Object.entries(cacheStats.formatDistribution).map(([format, count]: [string, any]) => (
                        <div key={format} className="flex items-center">
                          <div 
                            className="w-3 h-3 rounded-full mr-1"
                            style={{ 
                              backgroundColor: format === 'mp3' ? '#8884d8' : 
                                              format === 'ogg' ? '#82ca9d' : 
                                              format === 'wav' ? '#ffc658' : '#ff8042'
                            }}
                          ></div>
        ...
        <div className="space-y-6">
          {/* Configuration Panel */}
          <ConfigurationPanel
            optimizerType={OptimizerType.VOICE}
            config={config}
...
export default VoiceOptimizerView;